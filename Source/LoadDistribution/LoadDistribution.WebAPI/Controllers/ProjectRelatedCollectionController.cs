  ...
  where TDTO : class, IProjectRelatedDTO
{
      ...
      #region Methods
      [HttpGet("All")]
      public virtual async Task<IActionResult> GetAllAsync([FromQuery] int projectId)
      {
            IList<TEntity> entities;
            if (typeof(TEntity).IsAssignableTo(typeof(ICreateble)))
            {
                  entities = await _repository.ListAsync(sortExpression: entity => ((ICreateble)entity).Created, filterExpression: entity => entity.ProjectId == projectId);
            }
            else
            {
      ...
      [HttpGet("Paged")]
      public virtual async Task<IActionResult> GetPaged([FromQuery] int projectId, [FromQuery] int pageNumber, [FromQuery] int pageSize)
      {
            Paged<TEntity> entities;
            if (typeof(TEntity).IsAssignableTo(typeof(ICreateble)))
            {
                  entities = await _repository.PagedAsync(pageNumber, pageSize, sortExpression: entity => ((ICreateble)entity).Created, filterExpression: entity => entity.ProjectId == projectId);
            }
            else
            {
