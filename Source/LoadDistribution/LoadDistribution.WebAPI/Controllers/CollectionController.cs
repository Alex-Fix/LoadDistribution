  ...
  where TDTO : class, IDTO
{
      ...
      public virtual async Task<IActionResult> GetAllAsync()
      {
            ...
            {
                  entities = await _repository.ListAsync(sortExpression: entity => ((ICreateble)entity).Created);
            }
            else
            {
      ...
      [HttpGet("Paged")]
      public virtual async Task<IActionResult> GetPagedAsync([FromQuery] int pageNumber, [FromQuery] int pageSize)
      {
            Paged<TEntity> entities;
            if (typeof(TEntity).IsAssignableTo(typeof(ICreateble)))
            {
                  entities = await _repository.PagedAsync(pageNumber, pageSize, sortExpression: entity => ((ICreateble)entity).Created);
            }
            else
            {
